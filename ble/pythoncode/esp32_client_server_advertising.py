...
writefile1 = open("data.txt", '+r')
writefile1.truncate(0)
...
class dataCharacteristic1(bluetooth_gatt.Characteristic):
    temperature = 0
    delta = 0
    notifying = False
    ...
    def WriteValue(self, value, options):
        ...
        text = bytes(value).decode('utf8')
        self.value = text
        print(text)
        writefile1.writeline(text)
...
# temperature_period characteristic not implemented to keep things simple
    def __init__(self, bus, path_base, index):
        ...
        self.add_characteristic(dataCharacteristic1(bus, 0, self))
...
class Advertisement(dbus.service.Object):
    ...
    def GetAll(self, interface):
        if interface != bluetooth_constants.ADVERTISEMENT_INTERFACE: raise bluetooth_exceptions.InvalidArgsException()
...
#Where you can set what you want to do when a server device (in this case ESP32) connects to the peripheral

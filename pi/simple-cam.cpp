 ...
 */
// buffer mapping ---------------
void *mapFrameBuffer(FrameBuffer *buffer) {
    if (!buffer) {
        return nullptr;
    }

    // Obtain the file descriptor associated with the buffer
    int fd = buffer->fd();

    // Get the length of the buffer to map
    size_t length = buffer->length();  // You need to define how to get the buffer length

    // Map the buffer
    void *mappedData = mmap(nullptr, length, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
...
/**
 * Unmaps a FrameBuffer from the application's address space.
 *
 * @param buffer The FrameBuffer to unmap.
 * @param mappedData The pointer to the mapped memory region.
 * @return 0 on success, -1 on failure.
 */
int unmapFrameBuffer(FrameBuffer *buffer, void *mappedData) {
...
static void processRequest(Request *request)
{
	...
	for (auto bufferPair : buffers) {
		...
		std::cout << std::endl;
		void *data = mapFrameBuffer(buffer);
		if (!data) {
		...
		// Set up libjpeg structures and parameters
		struct jpeg_compress_struct cinfo;
		struct jpeg_error_mgr jerr;
		cinfo.err = jpeg_std_error(&jerr);
		jpeg_create_compress(&cinfo);

		// Set up the output file with the 'images/' directory
		std::string filename = std::string(dir) + "/frame_" + std::to_string(metadata.sequence) + ".jpg";
		FILE *outfile = fopen(filename.c_str(), "wb");
		if (!outfile) {
				std::cerr << "Failed to open " << filename << std::endl;
				unmapFrameBuffer(buffer);  // Assuming you have a corresponding unmap function
				continue;
		}

		jpeg_stdio_dest(&cinfo, outfile);

		// Set JPEG compression parameters for grayscale
		cinfo.image_width = WIDTH;  // Set image width
		cinfo.image_height = HEIGHT; // Set image height
		cinfo.input_components = 1; // Set number of color components to 1 for grayscale
		cinfo.in_color_space = JCS_GRAYSCALE; // Set color space to grayscale

		jpeg_set_defaults(&cinfo);
		jpeg_set_quality(&cinfo, JPEG_QUALITY, TRUE); // Set JPEG quality (0-100)
		...
		while (cinfo.next_scanline < cinfo.image_height) {
				row_pointer = (JSAMPROW)((char *)data + cinfo.next_scanline * ROW_STRIDE);

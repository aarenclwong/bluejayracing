...
string outvec(vector<double> v) {
  ...
  if (v.size() > 0) out += to_string(v[v.size()]);
  return out;
}
...
int main(/*int argc, char* argv[]*/) {
  ...
  */
  ...
  const static int log_num = 860*60*5;
  const static string file = "vtech_script";
  int iter = 0;
  while(1) {
    std::ofstream temp;
    temp.open(file + to_string(iter), "w");
    for(int i = 0; i < log_num; i++) {
      ...
      try {
        temp << outvec(gps.read()) << "\n";
      } catch(exception &e) {
        temp << outvec(vector<double>(0,3)) << "\n";
      }
